...
let sprite = (loc, sheet, ~scale, ~flip, ~pos as (x, y)) =>
  Reprocessing.Draw.subImagef(
    sheet,
    ~texPos=(loc.x, loc.y),
    ~texWidth=loc.width,
    ~texHeight=loc.height,
    ~width=(if (flip) {(-1.)} else {1.}) *. scale *. float_of_int(loc.width),
    ~height=scale *. float_of_int(loc.height),
    ~pos=(
      if (flip) {
        x +. float_of_int(loc.width) *. scale;
      } else {
        x;
      },
      y
...
module Tiles = {
  ...
  let leaves_orange_transparent =
    sprite({x: 390, y: 650, width: 128, height: 128});
  ...
  let redstone_emerald_alt =
    sprite({x: 260, y: 1430, width: 128, height: 128});
